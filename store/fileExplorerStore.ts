...
import { fileStructure, FileItem } from '@/types/fileStructure';
...
export const useFileExplorer = create<FileExplorer>((set) => ({
  ...
  addFileByAI: (filePath: string, fileName: string) =>
    set((state) => {
      ...
      if (pathParts.length === 0) {
        return {
          fileExplorer: [...state.fileExplorer, { name: fileNameFromPath, type: 'file' }],
        };
      }
      ...
      const addFileToPath = (
        ...
        currentPath = currentPath ? `${currentPath}/${currentFolder}` : currentFolder;
        // Open the folder
      ...
      return {
        fileExplorer: addFileToPath(state.fileExplorer, pathParts, fileNameFromPath),
      };
    }),
  ...
  deleteFileExplorer: (fileName: string) =>
    set((state) => {
      ...
      if (!currentPath) {
        ...
        return {
          fileExplorer: state.fileExplorer.filter((item) => item.name !== fileName),
        };
      }
      ...
      const deleteFileFromPath = (items: FileItem[], path: string[]): FileItem[] => {
        if (path.length === 0) {
          // We've reached the target location, remove the file here
          return items.filter((item) => item.name !== fileName.split('/').pop());
        }
  ...
  renameFileExplorer: (fileName: string, newName: string) =>
    set((state) => {
      ...
      const renameItemInPath = (items: FileItem[], path: string[]): FileItem[] => {
        if (path.length === 0) {
          // We've reached the target location, rename the item here
          return items.map((item) =>
            item.name === fileName.split('/').pop() ? { ...item, name: newName } : item,
          );
        }
    ...
    set((state) => {
      ...
      const renameFolderInPath = (
        ...
        return items.map((item) => {
          ...
          if (depth === path.length - 1 && item.name === targetFolder && item.type === 'folder') {
          ...
          if (item.type === 'folder' && item.name === path[depth]) {
            return {
              ...item,
              children: renameFolderInPath(item.children || [], path, depth + 1),
            };
          }
  ...
  newFolderExplorer: (folderName: string) =>
    set((state) => {
      ...
      if (!currentPath) {
        // If no folder is open, add to root
        return {
          fileExplorer: [...state.fileExplorer, { name: folderName, type: 'folder', children: [] }],
        };
      }
      ...
      // Helper function to recursively find and update the target folder
      const addFolderToPath = (items: FileItem[], path: string[]): FileItem[] => {
        if (path.length === 0) {
